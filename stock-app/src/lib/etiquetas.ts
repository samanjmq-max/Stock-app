...
function generarBarcodeDataUrl(codigo: string): { dataUrl: string; widthMm: number } {
  ...
  JsBarcode(canvas, codigo, {
    ...
    displayValue: false,
    margin: 0,
    width: 1.6, // ancho de barra angosta, equivalente a ~0.42mm físicos
    height: 90,
  });
...
/**
 * Agrega UNA etiqueta (una página de 10x5cm) al documento PDF recibido.
 * Mismo formato que las etiquetas generadas para el catálogo completo:
 * descripción, ubicación, código de barras y número, todo en negrita.
 */
export function agregarEtiqueta(doc: jsPDF, datos: DatosEtiqueta, esPrimera: boolean) {
  ...
  const { dataUrl, widthMm } = generarBarcodeDataUrl(datos.codigo);
  doc.addImage(dataUrl, "PNG", (PAGE_W_MM - widthMm) / 2, 12, widthMm, 13);

  doc.setFontSize(11);
  doc.text(datos.codigo, PAGE_W_MM / 2, 29, { align: "center" });
}

/**
 * Genera y descarga un PDF con una etiqueta por cada elemento de la lista.
 */
